		...
		<div class="row">
			<div class="col-8">
				<div class="card">
					...
					<div class="card-body">
						<form action="<?= base_url() ?>admins/pengguna/tambah/tambah" id="form_pengguna" method="post" enctype="multipart/form-data">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control <?= form_error('inputUsername') ? 'is-invalid' : '' ?>" name="inputUsername" id="inputUsername" value="<?= set_value('inputUsername') ?>" placeholder="Masukan username">
								<div class="invalid-feedback"><?= form_error('inputUsername') ?></div>
							</div>
							<div class="form-group">
								<label for="inputName">Nama</label>
								<input type="text" class="form-control <?= form_error('inputName') ? 'is-invalid' : '' ?>" name="inputName" id="inputName" value="<?= set_value('inputName') ?>" placeholder="Masukan nama">
					...
					<div class="card-footer text-right">
						...
						<button type="button" class="btn btn-primary btn-tambah-pengguna"><i class="fas fa-save"></i> Simpan</button>
					</div>
					<!-- /.card-body -->
				</div>
				<!-- /.card -->
...
<script>
	...
	$('.btn-tambah-pengguna').on('click', function() {
		...
		$.ajax({
			...
			chace: false,
			beforeSend: function() {
				$('.btn-tambah-pengguna').attr('disabled', 'disabled');
				$('.btn-tambah-pengguna').html('<i class="fas fa-spinner fa-spin"></i> In Process');
			},
			complete: function() {
				$('.btn-tambah-pengguna').removeAttr('disabled', 'disabled');
				$('.btn-tambah-pengguna').html('<i class="fas fa-save"></i> Tambah');
			},
